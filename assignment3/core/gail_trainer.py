...
1. Use GAIL model so the discriminator takes action as input
...
class GAILTrainer(PPOTrainer):
    ...
    def generate_expert_data(self, envs, size=10000):
        ...
        expert.load_w(current_dir, suffix="expert")
    ...
    def compute_loss(self, sample):
        ...
        return loss, policy_loss.mean(), torch.mean(gail_rewards), torch.mean(dist_entropy), torch.mean(ratio)
    ...
    def update(self, rollout):
        ...
        for epoch in range(1, self.config.discriminator_epoch + 1):  # Run multiple epochs
            ...
            for (expert_data, agent_data) in zip(self.expert_dataloader, data_generator):
                ...
                discriminator_loss = discriminator_loss_func(agent_prediction, torch.zeros_like(agent_prediction)) + discriminator_loss_func(
            ...
            # print("Finish {}/{} epochs of training of the discriminator.".format(epoch, self.config.discriminator_epoch))
        ...
        for e in range(self.config.generator_epoch):
            ...
            # print("Finish {}/{} epochs of training of the generator.".format(e, self.config.generator_epoch))
