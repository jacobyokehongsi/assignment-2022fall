...
from core.gail_trainer import GAILTrainer, GAILConfig
...
parser.add_argument(
    ...
    default=10000,
    type=int,
    help="The number of transitions in expert dataset. Default: 10,000"
)
parser.add_argument(
    "--seed",
    default=0,
...
parser.add_argument(
    ...
    default=5,
    type=int,
    help="Number of epochs for training discriminator in each training iteration. Default: 5"
)
parser.add_argument(
    "--generator-epoch",
    default=10,
    type=int,
    help="Number of epochs for training generator (policy) in each training iteration. Default: 10"
)
parser.add_argument(
    "--generator-lr",
    default=-1,
    type=float,
    help="Learning rate for generator (policy). Default: 1e-4"
)
parser.add_argument(
    "--discriminator-lr",
    default=-1,
...
if __name__ == '__main__':
    ...
    config.generator_epoch = args.generator_epoch

    if args.generator_lr != -1:
        config.generator_lr = args.generator_lr

    config.discriminator_epoch = args.discriminator_epoch

    if args.discriminator_lr != -1:
    ...
    trainer.generate_expert_data(envs=envs, size=args.expert_dataset_size)
    ...
    while True:  # Break when total_steps exceeds maximum value
        ...
        with sample_timer:
            for index in range(config.num_steps):
                ...
                with torch.no_grad():
                  actions, action_log_prob = trainer.compute_action(obs, deterministic=False)
                ...
                # Store samples
                trainer.rollouts.insert(to_tensor(obs), actions, action_log_prob, None, rewards, masks)

        # ===== Update Policy =====
        with update_timer:
            policy_loss, discriminator_loss, gail_reward, \
            dist_entropy, total_loss, norm, \
            adv, ratio = trainer.update(trainer.rollouts)
